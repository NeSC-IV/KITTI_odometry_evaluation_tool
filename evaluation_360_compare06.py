...
class kittiOdomEval():
    ...
    def loadPoses(self, file_name, toCameraCoord, format, seq_num, frame):
        '''
    ...
    def loadgtPoses(self, file_name, toCameraCoord, pose_ref, seq_num, frame):
        '''
            Each line in the file should follow one of the following structures
            time x y z rx ry rz rw
        '''
    ...
    def plotPath_2D_3(self, seq, poses_gt, poses_result2, poses_result3, plot_path_dir):
        '''
        ...
        png_title = "{}_comparepathsvg".format(seq)
        plt.savefig(plot_path_dir +  "/" + png_title + ".svg", bbox_inches='tight', pad_inches=0.1)
    ...
    def eval(self, toCameraCoord):
        ...
        # ate_errs2 = {}
        # ate_errs3 = {}       
        for seq in self.eval_seqs_range:
            seq_num = int(seq[3:])
            if(seq_num < 10):
            ...
            save_file_name = eva_seq_dir3 + '/{}.pdf'.format(seq)
            ...
            poses_result2 = self.loadPoses(pred_file_name2, toCameraCoord=False, format = self.pose_format, seq_num = seq_num, frame = self.frame)
            poses_result3 = self.loadPoses(pred_file_name3, toCameraCoord=False, format = self.pose_format, seq_num = seq_num, frame = self.frame)
          
            poses_gt = self.loadgtPoses(gt_file_name, toCameraCoord=True, pose_ref=poses_result3, seq_num = seq_num, frame = self.frame)
            ...
            print ("Average sequence translational RMSE (%):   {0:.4f}".format(ave_t_err3 * 100))
            print ("Average sequence rotational error (deg/m): {0:.4f}".format(ave_r_err3/np.pi * 180))
            ...
            self.plotPath_2D_3(seq, poses_gt, poses_result2, poses_result3, eva_seq_dir3)
            # self.plotError_segment(seq, avg_segment_errs2, avg_segment_errs3, eva_seq_dir3)
...
if __name__ == '__main__':
    ...
    # parser.add_argument('--sage_dir',type=str, default=dir_path + '/kitti360_sem', help='Directory path of the testing dataset')
    parser.add_argument('--kiss_dir',type=str, default=dir_path + '/kitti360_kissicp', help='Directory path of the testing dataset')
    parser.add_argument('--range_dir',type=str, default=dir_path + '/kitti360_range', help='Directory path of the testing dataset')
    ...
    parser.add_argument('--eva_seqs',   type=str, default='seq6',      help='The sequences to be evaluated, split by (,), or (*)')
    parser.add_argument('--pose_format',     type=str, default='tum',  help='Format of the pose file, kitti or tum')
    parser.add_argument('--frame',   type=int, default=4650,      help='The distance of the segment')
